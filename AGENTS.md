...
```bash
composer install
npm install                         # optional if using vite; not required for CDN mode
...
npm run dev                         # vite HMR only (optional)
composer run dev                    # concurrently: serve + queue:listen --tries=1 (vite-less, no npm required)
composer run dev:full               # concurrently: serve + queue:listen --tries=1 + vite (requires `concurrently` + npm)
...
npm run build                       # vite build -> public/build (optional, for production purge)
...
- **Frontend:** Blade + Tailwind (CDN mode by default). Entrypoints `resources/css/app.css`, `resources/js/app.js` (`vite.config.js:8`) kept for optional Vite build. Layouts `resources/views/layouts/app.blade.php` + `auth.blade.php` use `https://cdn.tailwindcss.com` + `css/sidebar.css`, `js/sidebar.js`, `js/navigation.js`, `js/toast.js` (AJAX nav + Alpine CDN) — no `@vite` required. Run `npm run build` only if you need purged production CSS. Views grouped by feature in `resources/views/{auth,users,roles,permissions,mcu,organization-*}`.
...
- **Vite:** `tailwindcss()` plugin is required; `public/build` and `public/hot` are gitignored. Run `npm run dev` alongside `php artisan serve` during UI work or use `composer run dev:full`. `composer run dev` is vite-less (CDN mode).
