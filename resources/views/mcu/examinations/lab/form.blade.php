...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
    <h2 class="text-2xl font-bold mb-2">Input Hasil Laboratorium</h2>
    ...
    <form action="{{ route("mcu-examinations.lab.store", $mcuRegistration) }}" method="POST">
        ...
        <table class="min-w-full divide-y divide-gray-200 mb-6">
            ...
            <tbody>
                @foreach($mcuRegistration->examLabs as $exam)
                <tr>
                    <td class="px-6 py-4 font-medium">{{ $exam->lab->name }}</td>
                    ...
                    <td class="px-6 py-4">
                        <input type="text" name="results[{{ $exam->id }}]" value="{{ $exam->result_value }}" class="shadow border rounded w-full py-2 px-3 text-gray-700" placeholder="Masukkan hasil...">
                    </td>
                </tr>
                @endforeach
            </tbody>
        ...
        <div class="flex justify-end">
            <a href="{{ route("mcu-examinations.lab.index") }}" class="text-gray-600 mr-4 mt-2">Batal</a>
