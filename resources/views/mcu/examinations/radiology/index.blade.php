@extends('layouts.app')
@section('title', 'Antrian Penunjang Non-Lab')
        ...
        <div class="px-5 py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="flex items-center gap-3">
                <div>
                    ...
                    <h2 class="text-xl font-bold mt-1 text-white">Antrian Penunjang Medis Non-Lab</h2>
                    <p class="text-xs mt-0.5 text-slate-400">Daftar pasien untuk pemeriksaan non-lab (EKG, Thorax, USG, dll).</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-5 py-4 bg-slate-50 border-b border-slate-200">
            <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
                <i class="fas fa-clipboard-list text-teal-600"></i> Daftar Antrian Non-Lab Pasien
            </h3>
            </div>
            ...
            @forelse($registrations as $i => $reg)
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <div>
                                ...
                                <span class="text-[10px] text-teal-600 font-medium flex items-center gap-1 mt-0.5">
                                    <i class="fas fa-heart-pulse text-[9px]"></i>
                                    {{ $reg->examRadiologies->count() }} Parameter Non-Lab
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            <a href="{{ route('mcu-examinations.radiology.form', $reg) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-bold text-white shadow-sm hover:shadow transition-all" style="background-color: #0f766e !important;">
                                <i class="fas fa-heart-pulse text-[10px]"></i> Isi Hasil Non-Lab
                            </a>
                        </td>
                    </tr>
            @empty
