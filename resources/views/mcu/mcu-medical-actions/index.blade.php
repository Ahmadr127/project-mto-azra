...
@section('title', 'Kelola Master Tindakan')
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold text-gray-900">Kelola Master Tindakan</h2>
                <a href="{{ route('mcu-medical-actions.create') }}" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
        ...
        <x-data-table :paginator="$mcu_medical_actions" :columns="$columns" :showNumber="true" searchMode="manual">
            @forelse($mcu_medical_actions as $i => $item)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ $mcu_medical_actions->firstItem() + $i }}</td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('mcu-medical-actions.edit', $item) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                            <form action="{{ route('mcu-medical-actions.destroy', $item) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
