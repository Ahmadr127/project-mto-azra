...
<div class="w-full mx-auto">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <x-data-table :paginator="$mcu_radiologies" :columns="$columns" :showNumber="true" searchMode="manual">
            @forelse($mcu_radiologies as $i => $item)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ $mcu_radiologies->firstItem() + $i }}</td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('mcu-radiologies.edit', $item) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                            <form action="{{ route('mcu-radiologies.destroy', $item) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
