...
@section('title', 'Kelola Master Pemeriksaan Fisik')
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold text-gray-900">Kelola Master Pemeriksaan Fisik</h2>
                <a href="{{ route('mcu-physical-exams.create') }}" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                    Tambah Master Fisik
                </a>
            </div>
        ...
        <x-data-table :paginator="$mcu_physical_exams" :columns="$columns" :showNumber="true" searchMode="manual">
            @forelse($mcu_physical_exams as $i => $item)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ $mcu_physical_exams->firstItem() + $i }}</td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('mcu-physical-exams.edit', $item) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                            <form action="{{ route('mcu-physical-exams.destroy', $item) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
