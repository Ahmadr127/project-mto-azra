...
<div class="w-full">
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Perbarui data <span class="font-semibold text-gray-700">{{ $patient->name }}</span> ({{ $patient->patient_code }})</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('patients.show', $patient) }}" class="hidden sm:inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                Lihat Detail
            </a>
    ...
    <form action="{{ route('patients.update', $patient) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        @include('mcu.patients._form', ['patient' => $patient])
