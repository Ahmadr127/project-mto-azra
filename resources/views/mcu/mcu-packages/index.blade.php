...
@php
    $columns = [
        ...
        ['key' => 'filter_name', 'label' => 'Nama Paket', 'type' => 'text', 'placeholder' => 'Nama...', 'searchable' => true],
        ['key' => 'harga', 'label' => 'Harga Dasar', 'searchable' => false],
        ['key' => 'item', 'label' => 'Total Item', 'searchable' => false],
...
<div class="w-full mx-auto">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <x-data-table :paginator="$mcu_packages" :columns="$columns" :showNumber="true" searchMode="manual">
            @forelse($mcu_packages as $i => $item)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ $mcu_packages->firstItem() + $i }}</td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">Rp {{ number_format($item->base_price, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ $item->items()->count() }} Item</td>
            ...
            @empty
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-gray-400">
                            ...
                            <p class="text-sm">Tidak ada paket</p>
                        </td>
                    </tr>
            @endforelse
        </x-data-table>
