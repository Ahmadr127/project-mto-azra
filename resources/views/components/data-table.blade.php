...
@if($isFullTable && $paginator)
    {{-- Full table mode with No + per-column search (Enter manual) --}}
    <form method="GET" action="{{ request()->url() }}" id="{{ $formId }}" class="w-full">
        {{-- Preserve per_page --}}
        @if(request('per_page'))
                            ...
                            <th class="px-2 py-2">
                                @if($searchable)
                                    ...
                                    @elseif($type === 'select' && isset($col['options']))
                                        <select name="{{ $key }}" class="w-full px-2 py-1.5 text-xs border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500 bg-white">
                                    ...
                                    @else
                                        <input type="text" name="{{ $key }}" value="{{ request($key, '') }}" placeholder="{{ $placeholder }}"
                                ...
                                @else
                                    @if($isLast)
                                        <div class="flex gap-1">
                                            <button type="submit" class="flex-1 inline-flex items-center justify-center px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold rounded-md">
                                                Filter
                                            </button>
